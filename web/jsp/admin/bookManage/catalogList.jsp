...
<body>
	...
	<h2 class="text-center">饰品分类信息管理</h2>
				...
				<c:when test="${!empty catalogList}">
					<c:forEach items="${catalogList}" var="i" varStatus="n">
						<tr>
							...
							<td>${i.catalogName}</td>
							<td class="noClick">
								<a class="btn btn-info btn-xs" href="jsp/admin/CatalogServlet?action=edit&id=${i.catalogId}">修改</a>
								<a class="btn btn-danger btn-xs" href="jsp/admin/CatalogServlet?action=del&id=${i.catalogId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
